<?php
  ...
  $pre = $pdo->prepare("SELECT * FROM schedules WHERE finish_at >= :n AND tags LIKE '%\"JMO\"%' ORDER BY start_at ASC;");
  $params = array(":n" => $now->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $schedules = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM information WHERE created_at >= :n AND tags LIKE '%\"JMO\"%' ORDER BY created_at DESC;");
  $params = array(":n" => $week->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $information = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM applications WHERE start_at <= :n1 AND finish_at >= :n2 AND tags LIKE '%\"JMO\"%' ORDER BY finish_at ASC;");
...
<head>
  ...
  <meta name="description" content="このページでは、日本数学オリンピック(JMO)に関する情報をまとめています。">
  <meta property="og:url" content="https://acaoly-inofficial.com/contests/mathematics/jmo.php">
  ...
  <meta property="og:title" content="日本数学オリンピック">
  ...
  <title>日本数学オリンピック | 学術オリンピック非公式まとめサイト</title>
...
<body>
  ...
  <div class="root-content-container">
    <div class="root-content-item">
      <a href="jmo/abstract.php"></a>
      <h3 class="root-content-item-title">大会概要</h3>
      <p class="root-content-item-text">JMOの簡単な内容はこちら</p>
    </div>
    
    <div class="root-content-item">
      <a href="jmo/past.php"></a>
      <h3 class="root-content-item-title">過去大会データ</h3>
      <p class="root-content-item-text">JMOの過去大会に関するデータはここから (過去問は別)</p>
    </div>

<?php
  ...
  $pre = $pdo->prepare("SELECT * FROM schedules WHERE start_at <= :m AND finish_at >= :n AND genre = 'ma' ORDER BY start_at ASC;");
  $params = array(":m" => $now->format("Y-m-d 23:59:59"), ":n" => $now->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $schedules = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM information WHERE created_at >= :n AND tags LIKE '%\"ma\"%'  ORDER BY created_at DESC;");
  $params = array(":n" => $week->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $information = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM applications WHERE start_at <= :n1 AND finish_at >= :n2 AND genre = 'ma' ORDER BY finish_at ASC;");
  $params = array(":n1" => $now->format("Y-m-d H:i:s"), ":n2" => $now->format("Y-m-d H:i:s"));
  $pre->execute($params);
  $applications = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM streams WHERE finish_at >= :n AND genre = 'ma'  ORDER BY start_at DESC;");
...
<head>
  ...
  <meta name="description" content="このページでは、学術オリンピックのうち数学分野についての情報をまとめています。">
  <meta property="og:url" content="https://acaoly-inofficial.com/contests/mathematics/index.php">
  ...
  <meta property="og:title" content="数学トップ">
  ...
  <title>数学トップ | 学術オリンピック非公式まとめサイト</title>
...
<body>
  ...
  <div class="root-content-container">
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">数学関係のお知らせはこちら</p>
    </div>
    
    <div class="root-content-item">
      <a href="schedule.php"></a>
      <h3 class="root-content-item-title">スケジュール</h3>
      <p class="root-content-item-text">数学関係の予定はこちら</p>
    </div>
    ...
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">数学関係の大会の過去問を一覧できます。</p>
    </div>
    ...
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">数学関係の大会等に関して、情報の補足や訂正等があれば気軽にご連絡ください。</p>
    </div>
  </div>
  ...
  <div class="root-genre-container">
    <div class="contest-link background-mathematics">
      <a href="jmo.php"></a>
      <p class="whiteletter-light color-mathematics">JMO</p>
      <div class="contest-link-content">
        <h3 class="light-underline">日本数学オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-mathematics">
      <a href="jjmo.php"></a>
      <p class="whiteletter-light color-mathematics">JJMO</p>
      <div class="contest-link-content">
        <h3 class="light-underline">日本ジュニア数学オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-mathematics">
      <a href="imo.php"></a>
      <p class="whiteletter-light color-mathematics">IMO</p>
      <div class="contest-link-content">
        <h3 class="light-underline">国際数学オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-mathematics">
      <a href="egmo.php"></a>
      <p class="whiteletter-light color-mathematics">EGMO</p>
      <div class="contest-link-content">
        <h3 class="light-underline">ヨーロッパ女子数学オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-mathematics">
      <a href="egmo_qual.php"></a>
      <p class="whiteletter-light color-mathematics">EGMOqual</p>
      <div class="contest-link-content">
        <h3 class="light-underline">EGMO日本代表一次選抜</h3>
      </div>
    </div>
    <div class="contest-link background-mathematics">
      <a href="apmo.php"></a>
      <p class="whiteletter-light color-mathematics">APMO</p>
      <div class="contest-link-content">
        <h3 class="light-underline">アジア太平洋数学オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-mathematics">
      <a href="hamamatsu.php"></a>
      <p class="whiteletter-light color-mathematics">浜松</p>
      <div class="contest-link-content">
        <h3 class="light-underline">高校生数学コンテスト in Hamamatsu</h3>
      </div>
    </div>
    <div class="contest-link background-mathematics">
      <a href="pilame.php"></a>
      <p class="whiteletter-light color-mathematics">PILAME</p>
      <div class="contest-link-content">
        <h3 class="light-underline">PILAME杯</h3>
      </div>
    </div>
  </div>

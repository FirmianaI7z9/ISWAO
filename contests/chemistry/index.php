<?php
  ...
  $pre = $pdo->prepare("SELECT * FROM schedules WHERE start_at <= :m AND finish_at >= :n AND genre = 'ch' ORDER BY start_at ASC;");
  $params = array(":m" => $now->format("Y-m-d 23:59:59"), ":n" => $now->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $schedules = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM information WHERE created_at >= :n AND tags LIKE '%\"ch\"%'  ORDER BY created_at DESC;");
  $params = array(":n" => $week->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $information = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM applications WHERE start_at <= :n1 AND finish_at >= :n2 AND genre = 'ch' ORDER BY finish_at ASC;");
  $params = array(":n1" => $now->format("Y-m-d H:i:s"), ":n2" => $now->format("Y-m-d H:i:s"));
  $pre->execute($params);
  $applications = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM streams WHERE finish_at >= :n AND genre = 'ch'  ORDER BY start_at DESC;");
...
<head>
  ...
  <meta name="description" content="このページでは、学術オリンピックのうち化学分野についての情報をまとめています。">
  <meta property="og:url" content="https://acaoly-inofficial.com/contests/chemistry/index.php">
  ...
  <meta property="og:title" content="化学トップ">
  ...
  <title>化学トップ | 学術オリンピック非公式まとめサイト</title>
...
<body>
  ...
  <div class="root-content-container">
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">化学関係のお知らせはこちら</p>
    </div>
    
    <div class="root-content-item">
      <a href="schedule.php"></a>
      <h3 class="root-content-item-title">スケジュール</h3>
      <p class="root-content-item-text">化学関係の予定はこちら</p>
    </div>
    ...
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">化学関係の大会の過去問を一覧できます。</p>
    </div>
    ...
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">化学関係の大会等に関して、情報の補足や訂正等があれば気軽にご連絡ください。</p>
    </div>
  </div>
  ...
  <div class="root-genre-container">
    <div class="contest-link background-chemistry">
      <a href="jcho.php"></a>
      <p class="whiteletter-dark color-chemistry">JChO</p>
      <div class="contest-link-content">
        <h3 class="dark-underline color-white">化学グランプリ</h3>
      </div>
    </div>
    <div class="contest-link background-chemistry">
      <a href="icho.php"></a>
      <p class="whiteletter-dark color-chemistry">IChO</p>
      <div class="contest-link-content">
        <h3 class="dark-underline color-white">国際化学オリンピック</h3>
      </div>
    </div>
  </div>

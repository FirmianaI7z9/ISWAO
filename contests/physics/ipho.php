<?php
  ...
  $pre = $pdo->prepare("SELECT * FROM schedules WHERE finish_at >= :n AND tags LIKE '%\"IPhO\"%' ORDER BY start_at ASC;");
  $params = array(":n" => $now->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $schedules = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM information WHERE created_at >= :n AND tags LIKE '%\"IPhO\"%' ORDER BY created_at DESC;");
  $params = array(":n" => $week->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $information = $pre->fetchAll();

  //$pre = $pdo->prepare("SELECT * FROM applications WHERE start_at <= :n1 AND finish_at >= :n2 AND tags LIKE '%\"IPhO\"%' ORDER BY finish_at ASC;");
  //$params = array(":n1" => $now->format("Y-m-d H:i:s"), ":n2" => $now->format("Y-m-d H:i:s"));
  //$pre->execute($params);
  //$applications = $pre->fetchAll();

  //$pre = $pdo->prepare("SELECT * FROM streams WHERE finish_at >= :n AND tags LIKE '%\"IPhO\"%' ORDER BY start_at DESC;");
...
<head>
  ...
  <meta name="description" content="このページでは、国際物理オリンピック(IPhO)に関する情報をまとめています。">
  <meta property="og:url" content="https://acaoly-inofficial.com/contests/physics/ipho.php">
  ...
  <meta property="og:title" content="国際物理オリンピック">
  ...
  <title>国際物理オリンピック | 学術オリンピック非公式まとめサイト</title>
...
<body>
  ...
  <div class="basic-container">
    <p class="contest-title"><span class="circle-red"></span>国際物理オリンピック</p>
    <hr>
    <p class="contest-subtitle">International Physics Olympiad <span class="tag background-physics color-white">IPhO</span></p>
  </div>
  ...
  <div class="root-content-container">
    <div class="root-content-item">
      <a href="ipho/abstract.php"></a>
      <h3 class="root-content-item-title">大会概要</h3>
      <p class="root-content-item-text">IPhOの簡単な内容はこちら</p>
    </div>
    
    <div class="root-content-item">
      <a href="ipho/past.php"></a>
      <h3 class="root-content-item-title">過去大会データ</h3>
      <p class="root-content-item-text">IPhOの過去大会に関するデータはここから (過去問は別)</p>
    </div>
    
    <div class="root-content-item">
      <a href="../common/problems.php?c="></a>
      <h3 class="root-content-item-title">過去問一覧</h3>
      <p class="root-content-item-text">IPhOの過去問を掲載しています。</p>
    </div>

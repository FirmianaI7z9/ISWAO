<?php
  ...
  $pre = $pdo->prepare("SELECT * FROM schedules WHERE start_at <= :m AND finish_at >= :n AND genre = 'ph' ORDER BY start_at ASC;");
  $params = array(":m" => $now->format("Y-m-d 23:59:59"), ":n" => $now->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $schedules = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM information WHERE created_at >= :n AND tags LIKE '%\"ph\"%'  ORDER BY created_at DESC;");
  $params = array(":n" => $week->format("Y-m-d 00:00:00"));
  $pre->execute($params);
  $information = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM applications WHERE start_at <= :n1 AND finish_at >= :n2 AND genre = 'ph' ORDER BY finish_at ASC;");
  $params = array(":n1" => $now->format("Y-m-d H:i:s"), ":n2" => $now->format("Y-m-d H:i:s"));
  $pre->execute($params);
  $applications = $pre->fetchAll();

  $pre = $pdo->prepare("SELECT * FROM streams WHERE finish_at >= :n AND genre = 'ph'  ORDER BY start_at DESC;");
...
<head>
  ...
  <meta name="description" content="このページでは、学術オリンピックのうち物理分野についての情報をまとめています。">
  <meta property="og:url" content="https://acaoly-inofficial.com/contests/physics/index.php">
  ...
  <meta property="og:title" content="物理トップ">
  ...
  <title>物理トップ | 学術オリンピック非公式まとめサイト</title>
...
<body>
  ...
  <div class="genre-header-container gradient-physics color-white">
    <p>物理</p>
  </div>
  ...
  <div class="root-content-container">
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">物理関係のお知らせはこちら</p>
    </div>
    
    <div class="root-content-item">
      <a href="schedule.php"></a>
      <h3 class="root-content-item-title">スケジュール</h3>
      <p class="root-content-item-text">物理関係の予定はこちら</p>
    </div>
    ...
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">物理関係の大会の過去問を一覧できます。</p>
    </div>
    ...
    <div class="root-content-item">
      ...
      <p class="root-content-item-text">物理関係の大会等に関して、情報の補足や訂正等があれば気軽にご連絡ください。</p>
    </div>
  </div>
  ...
  <div class="root-genre-container">
    <div class="contest-link background-physics">
      <a href="jpho.php"></a>
      <p class="whiteletter-dark color-physics">JPhO</p>
      <div class="contest-link-content">
        <h3 class="dark-underline color-white">全国物理コンテスト 物理チャレンジ</h3>
      </div>
    </div>
    <div class="contest-link background-physics">
      <a href="ipho.php"></a>
      <p class="whiteletter-dark color-physics">IPhO</p>
      <div class="contest-link-content">
        <h3 class="dark-underline color-white">国際物理オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-physics">
      <a href="apho.php"></a>
      <p class="whiteletter-dark color-physics">APhO</p>
      <div class="contest-link-content">
        <h3 class="dark-underline color-white">アジア物理オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-physics">
      <a href="eupho.php"></a>
      <p class="whiteletter-dark color-physics">EuPhO</p>
      <div class="contest-link-content">
        <h3 class="dark-underline color-white">ヨーロッパ物理オリンピック</h3>
      </div>
    </div>
    <div class="contest-link background-physics">
      <a href="opho.php"></a>
      <p class="whiteletter-dark color-physics">OPhO</p>
      <div class="contest-link-content">
        <h3 class="dark-underline color-white">Online Physics Olympiad</h3>
      </div>
    </div>
  </div>
